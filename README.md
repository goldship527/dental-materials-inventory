...
2026-05-23時点で、商品写真ストレージ、管理者ユーザー管理、商品マスタ一括取り込み、発注先連絡先管理、発注書下書き、発注済み記録、簡易納品確認、バーコード入出庫と納品確認でのロット番号・有効期限保存、期限ロット一覧まで実装済みです。
...
- 発注済みステータス、発注済み日時、簡易納品確認、納品確認取り消し
