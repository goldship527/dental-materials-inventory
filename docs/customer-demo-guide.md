...
初回提案では、細かい管理機能を全部見せるより、日常業務の流れを一本道で見せると伝わりやすくなります。
...
- これは歯科材料、消耗品、備品の在庫確認と発注準備を支援するデモです。
- デモデータはすべて架空データです。
- 患者情報や実在クリニック情報は入れません。
- 現時点では、人が判断して、人が発注先へ送る運用を前提にしています。
...
- 今日の注意項目
- 在庫0、不足、発注予定、期限ロット、長期在庫、異常出庫検知
- 日常操作の入口

話すポイント:

- 朝、まずここを見れば「今日確認するもの」が分かります。
- 現場スタッフは出庫や不足確認へ、本部や管理者は管理画面へ進めます。
...
- 毎回長い理由を入力させず、日常使用はすばやく記録できます。
- 実際に作業したスタッフを履歴に残せるので、ログインユーザーとは分けて追跡できます。
...
- 最低在庫を下回った商品だけが表示される
- 納品待ちがある商品は重複発注に注意できる
- 不足商品を発注候補へ追加できる
- A4縦で印刷できる

話すポイント:

- 棚を見に行かなくても、不足している商品から確認できます。
- すでに納品待ちのものが見えるので、二重発注を減らせます。
...
- システムが外部へ自動送信するのではなく、人がFAX、メール、電話、LINEなどで送った後に記録を残します。
- 発注済み、納品待ち、納品済みが分かるので、あとから状況確認しやすくなります。
...
- クリニックごとの在庫数、不足数、在庫0件数、発注候補
- クリニック別詳細への移動
- 使用個数CSV出力

話すポイント:

- 5クリニック1法人のような運用でも、本部が各院の状況を横断確認できます。
- 現時点では本部画面は確認中心で、各院の在庫を直接編集する画面にはしていません。

### 6. 補助機能を短く紹介する

時間があれば、次を短く見せます。

- `/barcode/stock`: 担当者バーコードと商品バーコードを使った入出庫
- `/stock-lots`: ロット番号と有効期限の確認
- `/stocktake/sessions`: 棚卸セッション
- `/products`: 商品マスタとバーコード管理
- `/account/notifications`: 朝のダイジェスト通知設定
...
- 作業スタッフを選んだクイック出庫
- バーコード検索、バーコード出入庫
- 商品マスタ、発注先マスタ、バーコードの管理
- 商品写真1枚の登録
- 不足在庫から発注候補を作る
- 発注書下書きをブラウザ印刷する
- 発注済み、納品待ち、納品済みを記録する
- 簡易的な納品確認と、必要に応じた在庫反映
- ロット番号、有効期限、入出庫履歴の確認
- 棚卸セッションの開始、途中保存、確定、履歴確認
- 複数クリニックを本部ダッシュボードで横断確認する
- 使用個数CSVを出力する
- 長期在庫、ABC分析、推奨最低在庫、異常出庫検知を確認する
- 朝のダイジェスト通知設定を確認する
...
- バーコードを読み取っただけで在庫を自動変更すること
- 購入履歴から在庫数を自動反映すること
- 購入履歴から発注候補を自動作成すること
- 複数クリニックの切替画面
- 本部画面から各院の在庫を直接編集すること
- 医療機器や滅菌物の法的管理を目的とした厳密な管理機能
- 本番運用向けの契約、バックアップ、障害対応ルールの確定

### 安全側に倒している点

- 在庫が変わる操作は、利用者が確認して確定した場合だけ実行します。
- バーコード読み取りだけでは在庫数を変えません。
- 外部発注送信やメール送信は行わず、人が送った後の記録を残します。
- 患者情報を入れない運用を前提にします。
- 実データ運用前に、バックアップ、権限、サポート範囲を別途決める必要があります。
