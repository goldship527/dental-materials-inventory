...
2026-05-20時点で、フェーズ3-Qの商品マスタ新規作成、商品写真1枚登録、ホーム確認画面強化、公開デモ環境準備まで実装済み。

現時点の到達点:

在庫を確認し、不足を見つけ、発注候補を作り、商品・発注先・バーコード情報を最低限整備し、商品マスタを新規作成し、商品写真を1枚登録し、棚卸をセッションとして途中保存・確定・履歴確認できる状態。

外出先で画面を見せるための公開デモは、GitHub、Vercel、Supabase PostgreSQLの構成で作成済み。公開デモDBには架空商品50件、不足商品10件、デモログインユーザーを投入済み。
...
商品写真は現状ローカル保存前提のため、公開デモでは永続保存を期待しない。
...
7. 商品編集画面のフィールド別エラー表示
8. 発注先新規作成と在庫行追加フローの整理
