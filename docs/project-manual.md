...
2026-05-24時点で、商品マスタ新規作成、商品写真1枚登録、商品マスタ一括取り込み、発注先マスタ一括取り込み、発注先連絡先管理、管理者ユーザー管理、監査ログ、発注書下書き、発注済み記録、簡易納品確認、スタッフ担当者バーコード、本部ダッシュボードまで実装済み。

現時点の到達点:

在庫を確認し、不足を見つけ、発注候補を作り、発注先ごとの発注書下書きを確認し、発注済みと簡易納品確認まで記録できる状態。商品・発注先・バーコード情報を整備し、商品マスタと発注先マスタを一括取り込みでき、商品写真を1枚登録し、棚卸をセッションとして途中保存・確定・履歴確認できる。
...
- 在庫編集、発注候補編集、棚卸確定は行わない

今後の予定:

- クリニック名を押して、クリニック別詳細ダッシュボードへ移動する
- クリニック別詳細から、そのクリニックの在庫一覧、不足一覧、発注候補、入出庫履歴へ進めるようにする
...
3. 本部ダッシュボードからクリニック別詳細へ進む導線を検討する
4. 初期導入用の在庫行追加フローを整理する
5. 発注済み・納品確認フローを実運用に近いデータで確認する
6. バーコードスキャナー到着後の実機確認
7. 発注候補を発注書単位にまとめるワークフロー
8. 発注先CSVエクスポートや取り込み履歴を検討する
