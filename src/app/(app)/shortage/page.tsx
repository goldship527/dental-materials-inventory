...
export default async function ShortagePage({ searchParams }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-surface px-6 py-8 text-ink print:bg-white print:p-0">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 print:max-w-none print:gap-3">
        <header className="flex flex-col gap-3 border-b border-line pb-5 md:flex-row md:items-end md:justify-between print:border-black print:pb-3">
          <div>
            ...
            <p className="mt-2 text-sm text-muted print:text-xs print:text-black">
              現在庫が最低在庫以下の商品だけを表示しています。発行日時: {generatedAt}
            </p>
          </div>
        ...
        <p className="hidden text-[10px] text-black print:block">
          印刷後は在庫棚と照合し、必要に応じて発注先へ確認してください。
        </p>
      </div>
    </main>
  );
