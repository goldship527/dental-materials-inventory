...
const field = "min-h-12 min-w-0 rounded-lg border border-muted bg-panel px-3 text-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent";
...
const categoryButton = "min-h-10 shrink-0 whitespace-nowrap rounded-lg border border-muted bg-panel px-3 text-sm font-semibold hover:border-accent hover:bg-subtle focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent";
const selectedCategoryButton = "min-h-10 shrink-0 whitespace-nowrap rounded-lg bg-accent px-3 text-sm font-semibold text-panel hover:bg-accentDeep focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2";
  ...
  return <>
    <section aria-label="納品待ち商品を絞り込む" className="grid gap-2 rounded-xl border border-line bg-panel p-3">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="grid gap-1 text-sm font-semibold">作業スタッフ<select className={field} value={staff.selectedStaffOperatorId} onChange={e => staff.selectStaffOperator(e.target.value)}><option value="">選択してください</option>{staffOperators.map(s => <option key={s.id} value={s.id}>{s.displayName}</option>)}</select></label>
      ...
      <div className="flex flex-wrap justify-between gap-2 text-sm text-muted"><p role="status">{filtered.length}商品 ／ 全{remaining.length}商品</p><p>担当: {staff.selectedStaffOperator?.displayName || "未選択"}</p></div>
    </section>
    ...
    {groups.map(([day, items]) => <section key={day} className="grid gap-2"><h2 className="text-lg font-semibold">{day} 発注 <span className="text-sm font-normal">／ {items.length}商品</span></h2><div className="grid items-start gap-2 sm:grid-cols-2 lg:grid-cols-4">{items.map(row => <ReceiptItem key={row.id} row={row} staffId={staff.selectedStaffOperatorId} onReceived={id => setReceived(ids => [...ids,id])} />)}</div></section>)}
  </>;
}
